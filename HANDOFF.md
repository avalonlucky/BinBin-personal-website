# Meridian Space 网站交接说明

更新时间：2026-08-18

## 2026-08-23 · 手机端不是桌面端堆叠版

案例页移动端必须按真实使用场景重新编排，禁止把桌面三栏直接改成三个纵向大方块。文化墙案例的同级卡片组统一使用原生横向滑动、`scroll-snap` 和下一张露出；桌面横向滚轮流程在手机端改为原生触控滑动，不保留滚轮拦截或 transform 位移。360° 全景说明默认只显示编号与标题，详细文案通过卡片内的小三角展开，并可滚动、收起，避免长文覆盖小屏画面。章节 Tab 在手机端使用可横向滑动的紧凑控件，触控目标不小于 44px。上线前必须逐一验证全景拖动、说明展开/收起、每组横滑首尾、Tab 切换、对比滑块、视频播放与整页无横向溢出。

手机端 360° 序列不得沿用桌面连续拼接停帧：每次滑动只能前进或后退一张，松手必须自动吸附到完整单图，图片使用 `object-fit: contain`，任何状态都不能同时出现两张图片。横图在竖向容器内必须顶部对齐；左右箭头叠放在图片上，舞台高度须在每次切图、展开、收起及视口变化后，按图片、当前说明卡和索引的实际高度重新计算，禁止任何状态出现上下空黑块；仅用于桌面的角度标尺在手机端隐藏。说明卡不加描边，只用卡片内 44px 触控范围的小三角展开，不使用遮挡内容的大号文字按钮。移动端媒体覆盖规则需要使用与基础选择器相同或更高的 specificity，禁止因 `.case-page.is-culture ...` 等桌面规则权重更高而造成双栏被挤进窄屏。
...
- 品牌名称已从 `Estrela Studio` 改为 `Meridian Space`。
- 已移除品牌名称旁的 `TM` 标记。
- 浏览器标签页图标已替换为 `M`。
- 首屏左右文案已替换为中文主标题和副标题。
...
- 页面正文与代码里不再出现真实姓名，对外统一用 **Meridian**。
