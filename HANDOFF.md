...
手机端 360° 序列不得沿用桌面连续拼接停帧：每次滑动只能前进或后退一张，松手必须自动吸附到完整单图，图片使用 `object-fit: contain`，任何状态都不能同时出现两张图片。横图在竖向容器内必须顶部对齐，禁止把适配留白放在图片上方；仅用于桌面的角度标尺在手机端隐藏。说明卡不加描边，只用卡片内 44px 触控范围的小三角展开，不使用遮挡内容的大号文字按钮。移动端媒体覆盖规则需要使用与基础选择器相同或更高的 specificity，禁止因 `.case-page.is-culture ...` 等桌面规则权重更高而造成双栏被挤进窄屏。
