...
| `cs-outro` / `cs-contact` / `cs-ai-row` / `cs-summary` | 页尾行动区：项目总结弹层、邮箱↔电话切换、问问 AI |

### 写新详情页的注意事项

1. 页面在 `work/` 下，所有资源路径要用 `../`。
2. `body` 必须带 `class="case-page"`，色彩变量定义在这个类上。
3. 详情页**不带页脚**（`s-footer` 只在一级页面出现）。详情页以
   `cs-closing` + `cs-nav` 收尾，`cs-nav` 已加大底部留白当页面结尾。
   若要新增详情页，同样不要复制页脚，否则末尾会出现一整屏黑区。
...
9. 页尾行动区的联系方式、AI 提示词与目标站点都写在 `case.js` 顶部的
   ...
   注意：只有 ChatGPT 的 `?q=` 前填稳定可用，国产模型的参数不一定被识别，
   所以点击时**先把提示词写进剪贴板再打开对话页**，前填失效也能用。
   AI 图标必须用 `mask` + `currentColor` 上色——外部 SVG 里的 `currentColor`
   在 `<img>` 中不会继承父级颜色，会渲染成黑色，在深底上等于看不见。
