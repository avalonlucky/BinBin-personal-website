(() => {
  ...
  const paintEntrance = () => {
    ...
    const lightProgress = Math.min(1, progress / .62);
    root.style.setProperty('--culture-progress', progress.toFixed(3));
    root.style.setProperty('--culture-light-progress', lightProgress.toFixed(3));
    ...
    door.style.opacity = String(.68 * (1 - lightProgress));
  ...
  const allViews = [
    ['f6-lobby-bright.jpg', '前台 · 序章与定格', '品牌认知与来访记忆点', '作为空间动线的起点，门面墙既是引导访客进入展厅叙事的起点，亦是来访客户打卡与商务合影的标志性背景，实现企业身份识别与二次社交传播的双重作用。'],
